...
XLSX.openxlsx(config_output_file, mode="w") do xf
    ...
    output_gdp_for_2015_selected[2, 1] = "Status"
    output_gdp_for_2015_selected[2, 2] = "Country"
    output_gdp_for_2015_selected[2, 3] = "Household consumption expenditure (including Non-profit institutions serving households)"
    output_gdp_for_2015_selected[2, 4] = "General government final consumption expenditure"
    output_gdp_for_2015_selected[2, 5] = "Gross capital formation"
    output_gdp_for_2015_selected[2, 6] = "Net exports"
    function addToSelected(n, country)
        output_gdp_for_2015_selected[n, 2] = country.name
        household_consumption_expenditure = getIndicator(country, "Household consumption expenditure (including Non-profit institutions serving households)")
        general_government_final_expenditure = getIndicator(country, "General government final consumption expenditure")
        gross_capital_formation = getIndicator(country, "Gross capital formation")
        country_imports = getIndicator(country, "Imports of goods and services")
        ...
        if !ismissing(household_consumption_expenditure) && !ismissing(general_government_final_expenditure) && !ismissing(gross_capital_formation) && !ismissing(country_imports) && !ismissing(country_exports)
            ...
            year = 2015
            if haskey(household_consumption_expenditure_value, year)
                output_gdp_for_2015_selected[n, 3] = household_consumption_expenditure_value[year]
            end
            if haskey(general_government_final_expenditure_value, year)
                output_gdp_for_2015_selected[n, 4] = general_government_final_expenditure_value[year]
            end
            if haskey(gross_capital_formation_value, year)
                output_gdp_for_2015_selected[n, 5] = gross_capital_formation_value[year]
            end
            if haskey(country_imports_value, year) && haskey(country_exports_value, year)
                output_gdp_for_2015_selected[n, 6] = country_exports_value[year] - country_imports_value[year]
            end
        else
            println("Error: ", country.name, " does not fulfill 4.1.5 (b).")
        end
    end
    n = 3
    output_gdp_for_2015_selected[n+0, 1] = "Developed economies"
    addToSelected(n + 0, getCountry(parsed_data, "Iceland"))
    addToSelected(n + 1, getCountry(parsed_data, "Finland"))
    addToSelected(n + 2, getCountry(parsed_data, "Netherlands"))
    output_gdp_for_2015_selected[n+3, 1] = "Economies in transition"
